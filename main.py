...
parser = argparse.ArgumentParser(description='Training and Evaluating config, Data:ImageNet')
...
parser.add_argument('--weight-decay', type=float, default=0.05,  # 0.05
...
def main():
    ...
    if args.local_rank == 0:
        _logger.info('Model %s created, param count: %d, training params count: %d' %
    ...
    if not os.path.exists(train_dir):
        ...
        exit(1)
    train_set = ImageDataset(train_dir)
    ...
    if not os.path.isdir(eval_dir):
        ...
        if not os.path.isdir(eval_dir):
            ...
            exit(1)
    test_set = ImageDataset(eval_dir)
...
def train_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            ...
            # ---------------------vvvvvv for vit vvvvv----------------------------------
            # fd_loss = cosine2_loss(model.module.patch_embed.f_x, 'bnc')
            # for j in range(12):
            #     fd_loss = fd_loss + cosine_loss(model.module.blocks[j].f_x)
